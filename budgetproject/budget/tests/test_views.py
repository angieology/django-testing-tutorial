...
# client helps mimic how client tests our views
...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        # name used to generate the slug, save method of Project model

    def test_project_list_GET(self):
        response = self.client.get(self.list_url)

        # we are able to access list flawlessly
    ...
    def test_project_detail_DELETE_deletes_expense(self):
        ...
        response = self.client.delete(self.detail_url, json.dumps({
            'id': 1 #id of expense we have created
        }))

        self.assertEquals(response.status_code, 204)
        # if we start with one, then delete, we shouldn't have any left
        self.assertEquals(self.project1.expenses.count(), 0)
